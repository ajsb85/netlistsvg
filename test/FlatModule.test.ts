...
test('split join', () => {
    ...
    // splits and joins are already added during construction (fromNetlist), so the
    // node count includes the expected one split and two joins.
    const numStartNodes = flatModule.nodes.length;
    flatModule.addSplitsJoins();
    const nodes = flatModule.nodes;
    // re-running the pass should not add any new nodes
